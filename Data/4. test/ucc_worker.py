...
# Stealth settings - FAST NINJA MODE with resilience
REQUEST_DELAY = 0.2  # Small delay between requests
REQUEST_VARIANCE = 0.1  # Slight variance
MAX_RESULTS_PER_NAME = 10
MAX_RETRIES = 5  # More retries for resilience
RETRY_DELAY_BASE = 2  # Start with 2s delay on error

# Create stealth session with connection pooling
...
session.headers.update({
    ...
    'Referer': 'https://publicsearchapi.floridaucc.com/',
    'Origin': 'https://publicsearchapi.floridaucc.com'
})
...
def stealth_delay():
    """Ninja mode - minimal delay"""
    delay = REQUEST_DELAY + random.uniform(0, REQUEST_VARIANCE)
    time.sleep(delay)

def search_debtor(name):
    print(f"  🔍 Searching UCC database for: \"{name}\"")
    url = f"{API_BASE}/Search"
    params = {**SEARCH_PARAMS, "text": name}

    for attempt in range(MAX_RETRIES):
        try:
            print(f"    📡 Sending request to Florida UCC API (attempt {attempt + 1}/{MAX_RETRIES})...")
            resp = session.get(url, params=params, timeout=60)
            
            if resp.status_code == 200:
                data = resp.json()
                debtors = data.get("payload", {}).get("debtors", [])[:MAX_RETRIES]
                if debtors:
                    print(f"    ✅ Found {len(debtors)} results from API")
                else:
                    print(f"    ❌ No results found in UCC database")
                return debtors
            elif resp.status_code == 429:
                wait_time = RETRY_DELAY_BASE * (2 ** attempt) + random.uniform(0, 10)
                print(f"    ⚠️  RATE LIMITED! API blocked us. Waiting {wait_time:.1f}s before retry...")
                update_status_error(f"Rate limited! Waiting {wait_time:.1f}s")
                time.sleep(wait_time)
            elif resp.status_code == 500:
                wait_time = 10 + random.uniform(0, 15)  # Longer wait for server errors
                print(f"    ⚠️  API SERVER ERROR (500)! Waiting {wait_time:.1f}s before retry...")
                update_status_error(f"API 500 error, waiting {wait_time:.1f}s")
                time.sleep(wait_time)
            else:
                wait_time = RETRY_DELAY_BASE * (2 ** attempt) + random.uniform(0, 5)
                print(f"    ❌ API error {resp.status_code}. Waiting {wait_time:.1f}s before retry...")
                update_status_error(f"API error {resp.status_code}")
                time.sleep(wait_time)
        except Exception as e:
            wait_time = RETRY_DELAY_BASE * (2 ** attempt) + random.uniform(0, 5)
            print(f"    ❌ Connection error: {type(e).__name__}. Waiting {wait_time:.1f}s before retry...")
            update_status_error(f"Connection error: {str(e)[:50]}")
            time.sleep(wait_time)
...
def get_filing_details(row_number, search_text):
    print(f"    📋 Fetching detailed filing information...")
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if resp.status_code == 200:
                ...
                print(f"    ✅ Got filing details")
                return payload
            elif resp.status_code == 429:
                wait_time = RETRY_DELAY_BASE * (2 ** attempt) + random.uniform(0, 10)
                print(f"    ⚠️  Rate limited on details! Waiting {wait_time:.1f}s...")
                update_status_error(f"Rate limited on details")
                time.sleep(wait_time)
            elif resp.status_code == 500:
                wait_time = 10 + random.uniform(0, 15)
                print(f"    ⚠️  API SERVER ERROR (500) on details! Waiting {wait_time:.1f}s...")
                update_status_error(f"API 500 error on details")
                time.sleep(wait_time)
            else:
