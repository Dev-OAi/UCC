...
export const DownloadSecurityModal: React.FC<DownloadSecurityModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
      <div
        className="w-full max-w-md bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden transform transition-all animate-in zoom-in-95 duration-300"
      ...
      >
        <div className="relative p-6 sm:p-8">
          ...
          <div className="flex flex-col items-center text-center space-y-4">
            <div className={`p-4 rounded-full ${lockoutUntil ? 'bg-red-50 text-red-600' : 'bg-blue-50 text-blue-600'}`}>
            ...
            <form onSubmit={handleSubmit} className="w-full space-y-4 pt-2">
              {lockoutUntil ? (
                <div className="flex flex-col items-center justify-center p-6 bg-slate-50 rounded-xl border border-slate-100 space-y-3">
                  <div className="flex items-center text-red-600 font-mono text-3xl font-bold">
                  ...
                  <p className="text-xs text-slate-400 font-medium uppercase tracking-widest">Wait before retry</p>
                </div>
              ) : (
                <div className="space-y-4">
                  <div className="relative group">
                    <input
                      ...
                      className={`w-full px-4 py-4 bg-slate-50 border-2 rounded-xl text-center text-xl font-mono tracking-[0.5em] focus:ring-4 focus:ring-blue-500/10 outline-none transition-all placeholder:tracking-normal placeholder:text-sm placeholder:font-sans ${
                        error ? 'border-red-200 focus:border-red-500' : 'border-slate-100 focus:border-blue-500'
                      }`}
                    />
                  </div>

                  {error && (
                    <p className={`text-sm font-medium ${attempts >= 3 ? 'text-red-600' : 'text-amber-600'}`}>
                      {error}
                    </p>
                  )}
                  ...
                  <button
                    type="submit"
                    className="w-full flex items-center justify-center space-x-2 py-4 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold transition-all shadow-lg shadow-blue-500/20 active:scale-[0.98]"
                  >
